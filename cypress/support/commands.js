Cypress.Commands.add('fillSignupFormAndSubmit', (email, password) => {
  cy.visit('/signup')
  cy.get('#email').type(email) //por id
  cy.get('#password').type(password, { log: false })
  cy.get('#confirmPassword').type(password, { log: false })
  cy.contains('button', 'Signup').click() //encontra por tipo button que tem o nome signup
  cy.get('#confirmationCode').should('be.visible') //verifica se o campo está visivel
})
